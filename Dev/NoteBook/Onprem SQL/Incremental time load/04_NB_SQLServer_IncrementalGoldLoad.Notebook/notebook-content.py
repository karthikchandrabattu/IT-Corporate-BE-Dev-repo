...
# META   "dependencies": {
# META     "lakehouse": {
# META       "default_lakehouse": "85c88f2a-28d8-44b3-9974-4b5dcc15c0e3",
# META       "default_lakehouse_name": "DE_LH_Gold",
...
# META         },
# META         {
# META           "id": "85c88f2a-28d8-44b3-9974-4b5dcc15c0e3"
# META         },
# META         {
...
GOLD_ABFSS_PATH = "abfss://eb87c50a-8bc7-40f8-bb1a-8a135398c6f4@onelake.dfs.fabric.microsoft.com/89412544-9ac3-4f39-a42d-09c035b68d13/Tables"
...
################################################### GETTING LIST OF SILVER SUCCESS TABLES ####################################################
try:
    SilverLogDetails = f"SELECT * FROM Log.ETLBatchSilverDetails WHERE BatchId = '{ETLBatchId}'"
    success_df = spark.read \
      .option(Constants.DatabaseName, "WH_DE_MetaData") \
        .synapsesql(SilverLogDetails)
...
def Read_bronze_data(silver_schema_name: str, bronze_table_name: str) -> tuple[int, DataFrame, str, str]:
    is_okay = 1
    error_message = None
    message = ''
    silver_df = None
    ...
    try:
        read_type = 'delta'
        relative_path = f"/{silver_schema_name}/{bronze_table_name}"
        bronze_abfss_path = f"{BASE_ABFSS_PATH}{relative_path}"
        silver_df = spark.read.format(read_type).load(bronze_abfss_path)
        message = f"Data successfully extracted (delta format) for Schema: {silver_schema_name}, Table: {bronze_table_name}"            
    except Exception as e2:
        is_okay = 0
        message = f"Error extracting data for Schema: {silver_schema_name}, Table: {bronze_table_name}"
        error_message = f"  Delta error: {str(e2)}"
        silver_df = None
    if silver_df is not None:
        silver_df = silver_df.dropDuplicates()

    return is_okay, silver_df, message, error_message
...
def data_type_casting(table_name, silver_df, config_mapping_schema_df):
    is_okay = 1
    error_message = None
        ...
        if table_name in distinct_tables:
            ...
            for row in table_config_sorted.collect():
                ...
                if column_name in silver_df.columns:
                    ...
                    silver_df = silver_df.withColumn(column_name, col(column_name).cast(spark_data_type))
                    msg = 'Data Type Casting was Completed'
                else:
                    print(f"Column '{column_name}' does not exist in the DataFrame. Skipping...")

            column_order = [row['COLUMN_NAME'] for row in table_config_sorted.collect() if row['COLUMN_NAME'] in silver_df.columns]
            silver_df = silver_df.select(*column_order)
    ...
    return is_okay, silver_df, msg, error_message
...
def adding_ETLWarehousing_Columns(silver_df:DataFrame):
        is_okay = 1
        error_message = None
        ...
        try:
            silver_df = silver_df.withColumn(
                                        "HashKey",
                                        md5(concat_ws("", *[col(c).cast("string") for c in silver_df.columns]))
                                    )
            silver_df = silver_df.withColumn("ETLLoadedDate", lit(canada_time)) \
                                .withColumn("ETLLoadedBy", lit(user_name)) \
                                .withColumn("ETLModifiedDate", lit(canada_time)) \
                                .withColumn("ETLModifedBy", lit(user_name)) 
            return is_okay, silver_df, msg, error_message 
        except Exception as e:
            is_okay = 0
            error_message = str(e)
...
############################################# USER DEFINED FUNCTION 4. PERFORM INCREMENTAL LOAD FOR GOLD LAYER  ########################################################################
def gold_data_incremental_Load(spark: SparkSession, silver_df: DataFrame, GoldTableName: str, GoldSchemaName: str, FullLoad,PrimaryKey):
    is_okay = 1
    gold_data_msg = ''
    error_message = None
    generic_path ="abfss://eb87c50a-8bc7-40f8-bb1a-8a135398c6f4@onelake.dfs.fabric.microsoft.com/85c88f2a-28d8-44b3-9974-4b5dcc15c0e3/Tables" 
    relative_path = f"{GoldSchemaName}/{GoldTableName}"
    ...
    try:
        try:
            ...
            is_okay = 0
            gold_data_msg = 'Error While Extracting PrimaryKey'
            error_message = str(e)
            return is_okay, gold_data_msg, error_message 
        ...
        if not DeltaTable.isDeltaTable(spark, absolute_path):
            try:
                ...
                gold_data_msg = 'Initial Load was completed'
                return is_okay, gold_data_msg, error_message

            except Exception as e:
                is_okay = 0
                gold_data_msg = 'Error While Performing Initial Write'
                error_message = str(e)
                return is_okay, gold_data_msg, error_message
            ...
            try:
                dataframe.write.mode("overwrite") \
                    ...
                    .save(absolute_path)
                if overwrite ==1:
                    gold_data_msg = 'No Primary Key performing Full Load'
                else:
                    gold_data_msg = 'performing Full Load'
                return is_okay, gold_data_msg, error_message
            except Exception as e:
                is_okay = 0
                gold_data_msg = 'Error While Performing Full Load'
                error_message = str(e)
                return is_okay, gold_data_msg, error_message
            ...
            try:
                try:
                    ...
                    is_okay = 0
                    gold_data_msg = 'Error While Loading the target tables'
                    error_message = str(e)
                    return is_okay, gold_data_msg, error_message
                ...
                try:
                    ...
                    is_okay = 0
                    gold_data_msg = 'Error While Getting new/updated records'
                    error_message = str(e)
                    return is_okay, gold_data_msg, error_message
                ...
                try:
                    ...
                    is_okay = 0
                    gold_data_msg = 'Error While Deleting old versions'
                    error_message = str(e)
                    return is_okay, gold_data_msg, error_message
                ...
                try:
                    ...
                    gold_data_msg = 'Merge and append operations completed ' + f'New Inserts: {new_count}'
                    return is_okay, gold_data_msg, error_message

                except Exception as e:
                    is_okay = 0
                    gold_data_msg = 'Error While Loading new/updated records'
                    error_message = str(e)
                    return is_okay, gold_data_msg, error_message

            except Exception as e:
                is_okay = 0
                gold_data_msg = 'Error in merge operation'
                error_message = str(e)
                return is_okay, gold_data_msg, error_message

    except Exception as e:
        is_okay = 0
        gold_data_msg = 'Unexpected error in incremental load'
        error_message = str(e)
        return is_okay, gold_data_msg, error_message
...
def update_table_level_log(log_dict,  EndTime=None, DurationInSec=None,
                            BronzeDataRead=None, DataTypeCasting=None, SilverCount=None,
                         GoldCount=None,
                            GoldDataLoad=None, 
    ...
    if GoldCount is not None:
        log_dict["GoldCount"] = GoldCount
    if SilverCount is not None:
        log_dict["SilverCount"] = SilverCount
    if GoldDataLoad is not None:
        log_dict["GoldDataLoad"] = GoldDataLoad
...
def process_table(row, InformationSchema_df,ETLBatchId):
    ...
    msg = None 
    gold_count, silver_count, source_delete_count, updated_row_count, inserted_row_count = None, None, None, None, None
    ...
    GoldSchemaName = row["GoldSchemaName"]
    GoldTableName = row['GoldTableName']
    ...
    ####################################### 1. BRONZE DATA READ ##############################################
    is_okay, silver_df, msg, error_message = Read_bronze_data( BronzeSchemaName,SourceTableName)
    if silver_df is None or is_okay ==0:
    ...
    if bronze_count == 0:
        ...
        TableLevelLog = update_table_level_log(TableLevelLog,BronzeDataRead=msg,SilverCount= bronze_count)
    ...
    is_okay, silver_df, msg, error_message = data_type_casting(SourceTableName, silver_df, data_typecasting_df)
    if(is_okay == 0):
        TableLevelLog = update_table_level_log(
                        TableLevelLog,
                        EndTime=datetime.now(timezone.utc),
    ...
    is_okay, silver_df, msg, error_message = adding_ETLWarehousing_Columns(silver_df)
    if(is_okay == 0):
        TableLevelLog = update_table_level_log(
                            TableLevelLog,
                            EndTime=datetime.now(timezone.utc),
    ...
    is_okay, msg , error_message= gold_data_incremental_Load(spark,silver_df, GoldTableName, GoldSchemaName,FullLoad, PrimaryKey)
    if(is_okay == 0):
        TableLevelLog = update_table_level_log(
                            TableLevelLog,
                            EndTime=datetime.now(timezone.utc),
    ...
    try:
        silver_delta_table = DeltaTable.forPath(spark, f"{GOLD_ABFSS_PATH}/{GoldSchemaName}/{GoldTableName}")
        silver_count = silver_delta_table.toDF().count()
        TableLevelLog = update_table_level_log(TableLevelLog, GoldCount=silver_count)
    except Exception as e:
...
if ConfigETL_df.count() > 0:
    ...
    schema = StructType([
        ...
        StructField("GoldCount", StringType(), True),
        StructField("GoldDataLoad", StringType(), True),
    ...
    spark_df = spark_df.withColumn("GoldCount", col("GoldCount").cast(LongType()).cast(StringType()))
    spark_df = spark_df.withColumnRenamed("LogHeaderId","BatchId")
    spark_df.write.option("overwriteSchema","true").mode("append").synapsesql("WH_DE_MetaData.Log.ETLBatchGoldDetails") 
else:
